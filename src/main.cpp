...
void *UMM_MALLOC_CFG_HEAP_ADDR = (void *)0x70000000;
uint32_t UMM_MALLOC_CFG_HEAP_SIZE = external_psram_size * 1048576; // MB to B
...
void setup()
{
	...
	// see https://github.com/rhempel/umm_malloc
	Serial.println("Start umm_malloc_test");delay(20);

	umm_init( );

	Serial.println("umm_malloc");delay(20);
	int * i10 =(int * ) umm_malloc(sizeof(uint32_t) * 10);

	Serial.println("memset");delay(20);
	memset(i10,0,sizeof(uint32_t) * 10);

	Serial.println("umm_free");delay(20);
	umm_free(i10);

	Serial.println("Start Pauls Test");delay(20);
